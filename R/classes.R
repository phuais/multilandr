...
#'  list with as many elements as previously inputted rasterlayers. Additionally, each element of
...
#'  by each point and radius, and the rasterlayer). The name of each element of each internal list
...
#'  given rasterlayer, P is the id of the point and R is the radius. For example, a plausible
...
#' @slot rast_names A list containing two data.frame with the names assigned by the user for the main rasterlayers
#' and extra rasterlayers defined in argument `rast_layer` and `ext_rast_layer` in [mland()].
#'
#' @examples
...
#' @slot rast_names A list with dataframes containing the names of the rasterlayers of the

...
#'
#' @details [pair_plots()] mainly relies on [GGally::ggpairs()] to generate pair plots
...
#' }
pair_plots <- function(x, raster = NULL, classes = NULL, radii = NULL, c_level = NULL, l_level = NULL,
  ...
  environment(.pair_plots_chk_args) <- environment()
  chk <- .pair_plots_chk_args()
  if(length(chk[[1]]) > 0)

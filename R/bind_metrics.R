.bind_metrics_chk_args <- function(){
...
#'
#' @details The function simply merge data.frame with metrics values, contained in an object of
#' class 'MultiLandMetrics' (returned by [metrics()]) with a data.frame with other data for each
...
#' classes to be included/excluded. If provided a character vector, [bind_metrics()] assumes that
...
#'
#' @examples
#' #' # Get sites names from ed_metrics and creates ad-hoc data.frame with random values of "richness"
#' # (the response variable). Only for the purpose of this example
#' sites <- ed_metrics@points$name
#' sampling_data <- data.frame(site = rep(sites, each = 10), richness = sample(1:500, 150))
#'
#' # With no filters, all columns with all metrics at all spatial scales are added to the sampling data
#' new_data <- bind_metrics(ed_metrics, sampling_data)
#'
#' # Subset for metrics of class "Forest", radius 5000 and metric "pland"
#' new_data <- bind_metrics(ed_metrics, sampling_data, classnames = TRUE, classes = "Forest",
#'                          radii = 3000, c_level = "pland")
#'
#' # In this format, the data.frame can be passed to a fitting model
#' fit <- lm(richness ~ r1_Forest_pland_3000, data = new_data)
bind_metrics <- function(x, data, raster = NULL, classes = NULL, radii = NULL,
  ...
  environment(.bind_metrics_chk_args) <- environment()
  chk <- .bind_metrics_chk_args()
  if(length(chk[[1]]) > 0)
